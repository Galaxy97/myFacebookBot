...
const saveReminders = (req) => {
  ...
  if (dateUnix > dataNow) {
    ...
    return `Create new reminders on ${nameDays[dayWeeks.getDay()]} at ${dayTimes.getHours()}:${dayTimes.getMinutes() < 10 ? '0' + dayTimes.getMinutes() : dayTimes.getMinutes()}`
  } else {
...
const deleteReminders = (req) => {
  if (req.body.queryResult.parameters.date || req.body.queryResult.parameters.time) {
    if (req.body.queryResult.parameters.date && req.body.queryResult.parameters.time) {
      ...
      const day = new Date(dayWeeks.getFullYear(), dayWeeks.getMonth(), dayWeeks.getDate(), dayTimes.getHours(), dayTimes.getMinutes(), 0).getTime()
